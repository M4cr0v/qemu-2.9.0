...
%ifarch %{ix86} x86_64
Requires: seabios-bin >= 1.10.2-3.el7_4.1
Requires: sgabios-bin
%endif
%ifnarch aarch64 s390x
Requires: seavgabios-bin >= 1.10.2-3.el7_4.1
Requires: ipxe-roms-qemu >= 20170123-1.git4e85b27.el7_4.1
%endif
%ifarch %{power64}
...
Source0: http://wiki.qemu.org/download/qemu-2.9.0.tar.xz
...
Source30: pc-slit.cphp
Source31: pc-slit.memhp
Source32: q35-slit.cphp
...
# copy test reference files to allow bios-tables-test pass
# TODO: remove this section on rebase to QEMU-2.10 as
# these files will be included in QEMU source tarball
cp %{SOURCE30} tests/acpi-test-data/pc/SLIT.cphp
cp %{SOURCE31} tests/acpi-test-data/pc/SLIT.memhp
cp %{SOURCE32} tests/acpi-test-data/q35/SLIT.cphp
cp %{SOURCE33} tests/acpi-test-data/q35/SLIT.memhp
...
install -p -m 0644 -t ${RPM_BUILD_ROOT}%{qemudocdir} Changelog README README.systemtap COPYING COPYING.LIB LICENSE %{SOURCE19} docs/qmp-spec.txt
...
%ifnarch s390x
    rm -f ${RPM_BUILD_ROOT}%{_datadir}/%{pkgname}/s390-ccw.img
%endif
...
%changelog
* Thu Mar  8 2018 Sandro Bonazzola <sbonazzo@redhat.com> - ev-2.9.0-16.el7_4.14.1
- Removing RH branding from package name

* Thu Jan 18 2018 Miroslav Rezanina <mrezanin@redhat.com> - rhev-2.9.0-16.el7_4.14
- kvm-fw_cfg-fix-memory-corruption-when-all-fw_cfg-slots-a.patch [bz#1534649]
- kvm-mirror-Fix-inconsistent-backing-AioContext-for-after.patch [bz#1535125]
- Resolves: bz#1534649
  (Qemu crashes when all fw_cfg slots are used [rhel-7.4.z])
- Resolves: bz#1535125
  (Mirror jobs for drives with iothreads make QEMU to abort with "block.c:1895: bdrv_attach_child: Assertion `bdrv_get_aio_context(parent_bs) == bdrv_get_aio_context(child_bs)' failed." [rhel-7.4.z])
...
- Resolves: bz#1513362
  (QEMU's AIO subsystem gets stuck inhibiting all I/O operations on virtio-blk-pci devices [rhel-7.4.z])

* Mon Nov 13 2017 Miroslav Rezanina <mrezanin@redhat.com> - rhev-2.9.0-16.el7_4.11
...
* Mon Oct 23 2017 Miroslav Rezanina <mrezanin@redhat.com> - rhev-2.9.0-16.el7_4.10
...
* Fri Sep 15 2017 Danilo Cesar Lemes de Paula <ddepaula@redhat.com> - rhev-2.9.0-16.el7_4.8
...
* Thu Sep 14 2017 Danilo Cesar Lemes de Paula <ddepaula@redhat.com> - rhev-2.9.0-16.el7_4.7
- kvm-vhost-Release-memory-references-on-cleanup.patch [bz#1490742]
- Resolves: bz#1490742
  (Hot-unplugging a vhost network device leaks references to VFIOPCIDevice's [rhel-7.4.z])

* Tue Sep 05 2017 Miroslav Rezanina <mrezanin@redhat.com> - rhev-2.9.0-16.el7_4.6
...
- kvm-vhost-ensure-vhost_ops-are-set-before-calling-iotlb-.patch [bz#1480446]
- kvm-vhost-user-unregister-slave-req-handler-at-cleanup-t.patch [bz#1480446]
- Resolves: bz#1480446
...
- Resolves: bz#1482515
  ([Tracing] capturing trace data failed [rhel-7.4.z])
- Resolves: bz#1482851
...
- Resolves: bz#1467509
  (CVE-2017-7539 Qemu quit abnormally when connecting to built-in NBD server [rhel-7.4.z])

* Thu Jul 20 2017 Miroslav Rezanina <mrezanin@redhat.com> - rhev-2.9.0-16.el7.3
...
* Tue Jul 18 2017 Miroslav Rezanina <mrezanin@redhat.com> - rhev-2.9.0-16.el7.2
...
* Mon Jul 17 2017 Miroslav Rezanina <mrezanin@redhat.com> - rhev-2.9.0-16.el7_4.1
- kvm-virtio-scsi-finalize-IOMMU-support.patch [bz#1471076]
- kvm-qemu-nbd-Ignore-SIGPIPE.patch [bz#1468108]
- Resolves: bz#1468108
  (CVE-2017-10664 qemu-kvm-rhev: Qemu: qemu-nbd: server breaks with SIGPIPE upon client abort [rhel-7.4.z])
- Resolves: bz#1471076
  (unbreak virtio-scsi for vIOMMU)

* Tue Jul 04 2017 Miroslav Rezanina <mrezanin@redhat.com> - 2.9.0-16.el7
- kvm-AArch64-Add-pci-testdev.patch [bz#1465048]
- Resolves: bz#1465048
  (AArch64: Add pci-testdev)

* Tue Jun 27 2017 Miroslav Rezanina <mrezanin@redhat.com> - 2.9.0-15.el7
...
* Fri Jun 23 2017 Miroslav Rezanina <mrezanin@redhat.com> - 2.9.0-14.el7
...
* Tue Jun 20 2017 Miroslav Rezanina <mrezanin@redhat.com> - 2.9.0-12.el7
...
* Fri Jun 16 2017 Miroslav Rezanina <mrezanin@redhat.com> - 2.9.0-11.el7
...
- kvm-target-ppc-Show-POWER9-in-cpu-help.patch [bz#1449969]
- Resolves: bz#1434706
...
- Rebase to QEMU 2.9.0 [bz#1387372]
- Resolves: bz#1387372
